...
import { Pin as PinIcon, Link2 as LinkIcon, Briefcase } from 'lucide-react';
...
const ActiveConversationCard = ({
  ...
  const lastBrowsedUrl = browsed_urls?.[browsed_urls.length - 1]?.url;
    ...
    >
      ...
      {lastBrowsedUrl && (
        <div className="mt-3 flex h-9 items-center justify-between rounded-lg bg-gray-50 p-2">
          <div className="flex w-full items-center gap-1">
            ...
            <span className="w-full truncate text-xs text-blue-500">{lastBrowsedUrl}</span>
          </div>
        </div>
      )}
      ...
      <div className="mt-3 flex gap-3">
        <div className="flex-1 rounded-lg bg-gray-50 p-2">
          ...
          <span className="text-sm text-gray-900">{company_revenue || '-'}</span>
        </div>
        <div className="flex-1 rounded-lg bg-gray-50 p-2">
          <span className="block text-xs text-gray-500">Employee count:</span>
          <span className="text-sm text-gray-900">{employee_count || '-'}</span>
        </div>
      </div>
