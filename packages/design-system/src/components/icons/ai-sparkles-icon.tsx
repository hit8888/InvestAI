...
const AiSparklesIcon = ({ className = '', width, height, viewBox, ...props }: Props) => {
  // Check if text-gray-900 is in className to use gray colors
  const isGray = className.includes('text-gray-900');

  // Gray color values
  const primaryFill = isGray ? '#111827' : '#4E46DC'; // gray-900 or primary
  const shimmerColor = isGray ? '#F3F4F6' : '#EDECFB'; // gray-100 or light purple
  const glowColor1 = isGray ? '#6B7280' : '#A9A4FF'; // gray-500 or purple
  const glowColor2 = isGray ? '#9CA3AF' : '#DCADFC'; // gray-400 or pink
  const glowColor3 = isGray ? '#6B7280' : '#6FC8FC'; // gray-500 or blue
  ...
  return (
    ...
    >
      ...
      <g mask="url(#mask0_10882_396853)">
        <circle cx="12.25" cy="12.9492" r="13.5" fill={primaryFill} />
        <g filter="url(#filter0_f_10882_396853)">
          <circle cx="5.5" cy="6.19922" r="7.5" fill={glowColor1} />
        </g>
        <g filter="url(#filter1_f_10882_396853)">
          <circle cx="19" cy="12.9492" r="7.5" fill={glowColor2} />
        </g>
        <g filter="url(#filter2_f_10882_396853)">
          <circle cx="7.75" cy="21.9492" r="7.5" fill={glowColor3} />
        </g>
      ...
      <defs>
        ...
        <linearGradient id="shimmerGradient" gradientUnits="userSpaceOnUse" x1="0" y1="1" x2="0" y2="23">
          <stop offset="0%" style={{ stopColor: shimmerColor, stopOpacity: 0 }} />
          <stop offset="30%" style={{ stopColor: shimmerColor, stopOpacity: 0.5 }} />
          <stop offset="40%" style={{ stopColor: shimmerColor, stopOpacity: 1 }} />
          <stop offset="60%" style={{ stopColor: shimmerColor, stopOpacity: 1 }} />
          <stop offset="70%" style={{ stopColor: shimmerColor, stopOpacity: 0.5 }} />
          <stop offset="100%" style={{ stopColor: shimmerColor, stopOpacity: 0 }} />
          <animateTransform
