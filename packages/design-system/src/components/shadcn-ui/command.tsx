...
>(({ className, showSearchIcon = true, containerClassName, ...props }, ref) => (
  <div className={cn('flex items-center border-b px-3 py-2', containerClassName)} cmdk-input-wrapper="">
    {showSearchIcon && <Search className="mr-2 h-4 w-4 shrink-0 opacity-50" />}
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        'placeholder:text-muted-foreground flex h-10 w-full rounded-md border border-system bg-transparent py-3 text-sm focus:border-none focus:outline-none focus:ring-2 focus:ring-system disabled:cursor-not-allowed disabled:opacity-50',
        className,
      )}
      {...props}
    />
